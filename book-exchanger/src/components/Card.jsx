...
function Card({ book }) {
  ...
  function handleDesc() {
    if (desc.length === 100) setdesc(book.description);
    else setdesc(book.description.substring(0, 100));
  }
  return (
    <div className="text-black w-[300px] flex flex-col gap-2 border border-gray-100 rounded shadow-lg bg-white">
      <div>
        <img
          src={book.img.url}
          alt="img"
          className="rounded h-[150px] w-[300px] object-cover"
        />
      </div>
      <div className="p-3 text-center flex-grow">
        ...
        {desc.length >= 100 ? (
          <p className="text-gray-500">
            {desc + "..."}
            {desc.length !== book.description.length ? (
              <button className="text-blue-400" onClick={handleDesc}>
                Read more
              </button>
            ) : (
              <button className="text-blue-400" onClick={handleDesc}>
